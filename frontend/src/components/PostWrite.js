...
function PostWrite() {
  ...
  const [uploadedImages, setUploadedImages] = useState([]); // {file, dataUrl}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 게시글 생성 (HTML 저장하여 이미지 base64 포함)
      ...
      // 첨부파일 업로드만 수행 (selectedFiles만 업로드, uploadedImages는 제외)
      ...
      alert(err.response?.data?.error || '작성에 실패했습니다.');
    }
  };
        ...
        <div className="form-card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-actions">
              ...
              <button type="button" className="cancel-btn" onClick={() => history.push('/posts')}>
                취소
              </button>
            </div>
          </form>
